...
When M4 completes, enable the following:

**CI workflow (`.github/workflows/ci.yaml`):**

- Add `dart_monty_wasm` to the `test` job matrix (`dart test --platform chrome --coverage`)
  - **Warning:** `dart test --platform chrome` will NOT work for WASM threads testing because the built-in test server does not serve `Cross-Origin-Opener-Policy` / `Cross-Origin-Embedder-Policy` headers required for `SharedArrayBuffer`. Use a custom test runner or proxy server that injects COOP/COEP headers.
- Add coverage check for `dart_monty_wasm` (>= 90% gate)
- Add `build-js-wrapper` job: `npm install && npm run build` in `packages/dart_monty_wasm/js/`
- Add `python-ladder-wasm-package` job: `tool/test_python_ladder.sh --paths=wasm-package --tiers=1-6` in headless Chrome
- Cache `node_modules` with `actions/setup-node` + `cache: npm`

**Pre-commit hooks (`.pre-commit-config.yaml`):**

- ~~Add `dart-test-wasm` hook~~: Do NOT add browser-based WASM tests as a pre-commit hook -- they are too slow and flaky for pre-commit. Keep WASM browser tests in CI and the `tool/test_wasm.sh` gate script only.
