...
- [ ] `MontyResolveFutures` progress variant
- [ ] `resolveFutures()` method on `MontyPlatform`
- [ ] `ExternalResult` type (Return, Error, Future)
- [ ] Allow `resume()` to return `ExternalResult::Future` to create futures
- [ ] Unit tests for new types and resume protocol

### 13.2 Rust C FFI

- [ ] Detect `ResolveFutures` tag in progress dispatch
- [ ] `monty_pending_future_call_ids` accessor
- [ ] `monty_resume_futures` with JSON results map
- [ ] Support `ExternalResult::Future` in resume path
- [ ] Rust unit tests

### 13.3 FFI Implementation

- [ ] `MontyFfi`: recognize `ResolveFutures` progress
- [ ] Construct `MontyResolveFutures` from native accessors
- [ ] `resolveFutures()` implementation
- [ ] Host returning `Future` from sync call -> VM creates awaitable
- [ ] Mock-based unit tests (>= 90% coverage)
- [ ] Integration tests: single await, gather, error propagation

### 13.4 WASM Implementation

- [ ] `MontyWasm`: recognize `ResolveFutures` from Worker
- [ ] `resolveFutures()` through JS bridge
...
- [ ] Dartdoc comments on `MontyResolveFutures`, `ExternalResult`
- [ ] Document the async execution flow: sync call -> return Future ->
      VM yields ResolveFutures -> host resolves -> VM resumes
- [ ] Code examples showing `asyncio.gather` with concurrent host resolution
- [ ] Document call_id correlation pattern
