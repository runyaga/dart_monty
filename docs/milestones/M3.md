...
When M3 completes, enable the following:

**CI workflow (`.github/workflows/ci.yaml`):**

- Add `dart_monty_ffi` to the `test` job matrix (`dart test --coverage`)
- Add coverage check for `dart_monty_ffi` (>= 90% gate, same pattern as platform_interface)
- Add `integration-native` job: build native lib, run `dart test --tags=integration` with `DYLD_LIBRARY_PATH`/`LD_LIBRARY_PATH`
  - **Important:** `LD_LIBRARY_PATH` (Linux) and `DYLD_LIBRARY_PATH` (macOS) must be set to the directory containing the built shared library so integration tests can find it at runtime
- Add `web-spike` job: `dart compile wasm` spike, headless Chrome verification
- Add `python-ladder` job: run `tool/test_python_ladder.sh --tiers=1-6` on native and WASM paths
- Add `snapshot-portability` job: native-to-WASM snapshot round-trip

**Pre-commit hooks (`.pre-commit-config.yaml`):**

- Add `dart-test-ffi` hook: `cd packages/dart_monty_ffi && dart test` (unit tests only, no native lib required)
