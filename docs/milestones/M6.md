...
When M6 completes, enable the following:

**CI workflow (`.github/workflows/ci.yaml`):**

- Add `dart_monty_web` to the `test` job matrix (`flutter test --platform chrome --coverage`)
- Add coverage check for `dart_monty_web` (>= 90% gate)
- Add `example-build-web` job: `flutter build web` in `example/` to verify the example app compiles for web
- Add `python-ladder-flutter-web` job: `tool/test_python_ladder.sh --paths=flutter-web --tiers=1-6` in headless Chrome
- Add COOP/COEP header verification step in web integration tests

**Pre-commit hooks (`.pre-commit-config.yaml`):**

- Add `flutter-test-web` hook: `cd packages/dart_monty_web && flutter test --platform chrome`
