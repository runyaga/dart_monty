...
- `packages/dart_monty_native` -- Flutter plugin for macOS + Linux
...
`packages/dart_monty_native/pubspec.yaml` (note `dartPluginClass`
...
- [x] Desktop package declares `dartPluginClass: DartMontyNative`
- [x] `registerWith()` static method
- [x] No method channels

### 5.2 Native Library Bundling

- [x] `packages/dart_monty_native/macos/dart_monty_native.podspec`
  - Use `vendored_libraries` for proper code signing
- [x] `packages/dart_monty_native/linux/CMakeLists.txt`
...
- [x] `tool/test_native.sh`:
  1. Build native library
  2. `flutter test packages/dart_monty_native`
...
- Add `dart_monty_native` to the `test` job matrix (`flutter test --coverage`)
- Add coverage check for `dart_monty_native` (>= 90% gate)
...
- Add `flutter-test-desktop` hook: `cd packages/dart_monty_native && flutter test`
