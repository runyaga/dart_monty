...
- [ ] Root pubspec uses `default_package` (remove existing `ffiPlugin: true` from macos/linux blocks)
- [ ] Desktop package has `flutter.plugin.implements`
- [ ] Desktop package declares `dartPluginClass: DartMontyDesktop`
...
- [ ] Update `NativeLibraryLoader` with fallback path strategy for bundled apps
...
- [ ] `SendPort`/`ReceivePort` communication with Request/Response ID matching
- [ ] Commands: run, start, resume, resumeWithError, snapshot, dispose
- [ ] External function callbacks via Isolate message passing
- [ ] `Isolate.addOnExitListener` -- complete pending Futures with `MontyException` on crash
- [ ] `Isolate.addErrorListener` -- surface Isolate errors to callers
- [ ] Use `TransferableTypedData` for snapshot `Uint8List` to avoid copy overhead
- [ ] Graceful disposal: drain pending work before killing Isolate
...
- [ ] Isolate crash recovery: verify pending Futures fail cleanly
