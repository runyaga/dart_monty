...
and date/time (`TimeOsCallHandler`). `composeOsHandlers()` composes
...
| Composite provider child map | `composeOsHandlers()` | `dispose()` iterates + dedup-disposes children | Composite `OsCallHandler` |
