...
group('G5-1: SignalCapture on EventLoop.channelStateSignal', () {
  ...
  test('records every state value including initial Idle', () async {
    ...
    expect(cap.values.first, isA<BridgeChannelIdle>());
    expect(cap.values.any((s) => s is BridgeChannelExecuting), isTrue);
    expect(cap.values.last, isA<BridgeChannelCompleted>());
  });

  test('calling dispose() on SignalCapture stops recording', () {
    // Use a plain signal to avoid the EventLoop Completed-state dispatch wart.
    final s = signal(0);
...
group('G5-3: Computed signal derived from EventLoop state', () {
  ...
  test('computed "isWaiting" tracks el waiting state', () async {
    final isWaiting = computed(() => el.channelStateSignal.value is BridgeChannelWaiting);
    final history = <bool>[];
...
''');
    ...
    // Children may complete fast; assert at least one was alive and all finished.
...
group('G5-6: FauxUi sees BridgeFunctionEmit from el_emit', () {
  ...
  test('each el_emit call produces a BridgeFunctionCallStart event', () async {
    await h.run('''
el_emit(value={'tick': 1})
el_emit(value={'tick': 2})
''');

    final emitCalls = ui.eventsOf<BridgeFunctionCallStart>()
...
group('G5-7: FauxUi.assertEventSequence', () {
  ...
  test('run starts with BridgeRunStarted and ends with BridgeRunFinished', () async {
    await h.run("ping()");

    // Check presence of key event types — strict ordering breaks on intermediate
    // events like BridgeStepStarted that appear between run/function events.
    ui.assertContains<BridgeRunStarted>();
    ui.assertContains<BridgeFunctionCallStart>();
